...
$(document).ready(function() {
    ...
    function onMouseMove(e) {
        ...
        // Calculate the percentage values based on mouse position
        let xPercentage = (e.clientX - rect.left) / rect.width * 100;
        let yPercentage = (e.clientY - rect.top) / rect.height * 100;

        // Apply the background-position
        container.style.backgroundPosition = `${xPercentage}% ${yPercentage}%`;
    }
    ...
    $(document).ready(function() {
        ...
        const $containCheckbox = $('#contain');
  
        // Function to handle checkbox clicks
        function handleCheckboxClick() {
          // Remove both classes initially
          $('.image-container').removeClass('bg-contain');
  
          // Check the state of the checkboxes and apply the appropriate class
          if ($containCheckbox.prop('checked')) {
            $('.image-container').addClass('bg-contain');
            $('.image-container').css('background-position','center');
          }
        }
  
        // Add event listeners to the checkboxes
        $containCheckbox.on('click', handleCheckboxClick);
      });
